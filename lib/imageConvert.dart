...
// Fill image buffer with plane[0] from YUV420_888
    ...
    final originalImage = imagelib.decodeImage(png);
    final height1 = originalImage!.height;
    final width1 = originalImage.width;
    late imagelib.Image? fixedImage;

    if (height1 < width1) {
      fixedImage = imagelib.copyRotate(originalImage, 90);
    }
    final path =
    join((await getTemporaryDirectory()).path, "${DateTime.now()}.jpg");
    print(path);
    File(path).writeAsBytesSync(imagelib.encodeJpg(fixedImage!));
    return imagelib.encodeJpg(fixedImage);
  } catch (e) {
    print(">>>>>>>>>>>> ERROR:" + e.toString());
  }
  return null;
